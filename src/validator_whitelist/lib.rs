...
mod validator_whitelist{
    ...
    impl ValidatorWhitelist{
        ...
        #[ink(message)]
        pub fn add_validator(&mut self,validator:AccountId)->Result<(),WhitelistError>{
            Ok(())
        }
        #[ink(message)]
        pub fn remove_validator(&mut self,validator:AccountId,slash:bool)->Result<(),WhitelistError>{
            Ok(())
        }
    }
