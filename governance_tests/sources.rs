...
pub fn hash_governance_council() -> String {
    let json = read_to_string("../deployments/development/governance_council/governance_council.json").unwrap();
    let artifact: Artifact = from_str(&json).expect("Should extract hash from governance_council.json");
    artifact.source.hash
}
...
pub fn transcoder_governance_council() -> Option<Rc<ContractMessageTranscoder>> {
    Some(Rc::new(
        ContractMessageTranscoder::load(PathBuf::from(
            "../deployments/development/governance_council/governance_council.json",
        ))
            .expect("Failed to create transcoder"),
    ))
}
...
pub fn bytes_governance_council() -> Vec<u8> {
    read("../deployments/development/governance_council/governance_council.wasm")
        .expect("Failed to find or read contract file")
}
