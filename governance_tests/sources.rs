...
pub fn hash_governance_council() -> String {
    let json =
        read_to_string("../deployments/development/governance_council/governance_council.json")
            .unwrap();
    let artifact: Artifact =
        from_str(&json).expect("Should extract hash from governance_council.json");
    artifact.source.hash
}
